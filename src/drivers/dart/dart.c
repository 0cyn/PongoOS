...
#define memalign(alignment, size) alloc_contig(size)
#define dma_wmb() __asm__ volatile("dmb oshst" ::: "memory")
#define is_heap(ptr) (!dart->locked && !dart->keep)
...
static void set32(uintptr_t addr, u32 value)
{
    write32(addr, read32(addr) | value);
}

static int poll32(uintptr_t addr, u32 mask, u32 target, u32 timeout)
{
    while (timeout--) {
        if ((read32(addr) & mask) == target)
            return 0;
    }
...
{
    write32(dart->regs + DART_T8020_STREAM_SELECT, dart->sid_mask);
    ...
    if (dart->locked || keep_pts) {
        for (int i = 0; i < dart->params->ttbr_count; i++) {
            ...
            if (ttbr & dart->params->ttbr_valid)
                dart->l1[i] = phystokv(FIELD_GET(dart->params->ttbr_addr, ttbr)
                                       << dart->params->ttbr_shift);
        }
    }
...
static bool dart_dt_compatible(dt_node_t *node, const char *target)
{
    size_t len = 0;
    const char *compatible = dt_prop(node, "compatible", &len);

    while (compatible && len) {
        size_t item_len = strnlen(compatible, len);
        if (item_len == strlen(target) && !memcmp(compatible, target, item_len))
            return true;
        if (item_len == len)
            break;
        compatible += item_len + 1;
        len -= item_len + 1;
    }
...
dart_dev_t *dart_init_from_dt_mapper(dt_node_t *mapper, u32 reg_index, bool keep_pts)
{
    ...
    size_t stream_len = 0;
    u32 *stream = dt_prop(mapper, "reg", &stream_len);
    dt_node_t *node = dt_node_parent(mapper);
    size_t reg_len = 0;
    u64 *reg = node ? dt_prop(node, "reg", &reg_len) : NULL;
    if (!stream || stream_len < sizeof(*stream) || *stream > UINT8_MAX || !reg ||
        reg_len < (reg_index + 1) * 2 * sizeof(*reg))
        return NULL;

    enum dart_type_t type;
    if (dart_dt_compatible(node, "dart,t8020"))
        type = DART_T8020;
    else if (dart_dt_compatible(node, "dart,t6000"))
        type = DART_T6000;
    else if (dart_dt_compatible(node, "dart,t8110"))
        type = DART_T8110;
    else
        return NULL;

    dart_dev_t *dart = dart_init(gIOBase + reg[reg_index * 2], *stream, keep_pts, type);
    size_t sids_len = 0;
    u32 *sids = dt_prop(node, "sids", &sids_len);
    if (dart && sids && sids_len >= sizeof(*sids))
        dart->sid_mask = *sids;
...
{
    size_t len = 0;
    u32 *phandle = consumer ? dt_prop(consumer, "iommu-parent", &len) : NULL;
    if (!phandle || len < sizeof(*phandle))
        return NULL;

    return dart_init_from_dt_mapper(dt_find_phandle(gDeviceTree, *phandle), reg_index, keep_pts);
}

u32 dart_get_error(dart_dev_t *dart)
{
    return read32(dart->regs + DART_T8020_ERROR);
}

u64 dart_get_error_address(dart_dev_t *dart)
{
    return read32(dart->regs + DART_T8020_ERROR_ADDR_LO) |
           ((u64)read32(dart->regs + DART_T8020_ERROR_ADDR_HI) << 32);
}
...
static u64 *dart_get_l2(dart_dev_t *dart, u32 idx)
{
    ...
    memset(tbl, 0, SZ_16K);
    cache_clean(tbl, SZ_16K);

    u64 offset = FIELD_PREP(dart->params->offset_mask,
                            vatophys_static(tbl) >> DART_PTE_OFFSET_SHIFT);

    dart->l1[ttbr][idx] = offset | DART_PTE_VALID;
    cache_clean(&dart->l1[ttbr][idx], sizeof(dart->l1[ttbr][idx]));
...
static int dart_map_page(dart_dev_t *dart, uintptr_t iova, uintptr_t paddr, u32 flags)
{
    ...
    if (l2[l2_index] & DART_PTE_VALID) {
        printf("dart: iova %lx already has a valid PTE: %llx\n", iova, l2[l2_index]);
        return -1;
    }
    ...
    cache_clean(&l2[l2_index], sizeof(l2[l2_index]));
...
{
    return dart_map_flags(dart, iova, paddr, len, 0);
}
...
void dart_free_l2(dart_dev_t *dart, uintptr_t iova)
{
    ...
    cache_clean(&dart->l1[ttbr][l1_index], sizeof(dart->l1[ttbr][l1_index]));
    free_contig(l2, SZ_16K);
}
...
static void *dart_translate_internal(dart_dev_t *dart, uintptr_t iova, int silent)
{
    ...
    u32 l2_index = (iova >> 14) & 0x7ff;
    u64 *l2 = phystokv(FIELD_GET(dart->params->offset_mask, dart->l1[ttbr][l1_index])
                        << DART_PTE_OFFSET_SHIFT);
...
u64 dart_search(dart_dev_t *dart, void *paddr)
{
    for (int ttbr = 0; ttbr < dart->params->ttbr_count; ++ttbr) {
        ...
        for (u32 l1_index = 0; l1_index < 0x7ff; l1_index++) {
            ...
            u64 *l2 = phystokv(FIELD_GET(dart->params->offset_mask,
                                         dart->l1[ttbr][l1_index])
                                << DART_PTE_OFFSET_SHIFT);
